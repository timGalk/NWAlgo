...
// Helper function to generate a timestamped filename
    fun generateTimestampedFilenamePDF(): String {
        ...
        return "MSA_report_${LocalDateTime.now().format(formatter)}.pdf"
    }

    fun generateMSAReport(result: MultipleAlignmentResult) {
        ensureOutputDirectoryExists()

        // Export the PDF
        val filePath = "$RESULTS_FOLDER/${generateTimestampedFilenamePDF()}"
        val file = File(filePath)
        ...
        document.add(Paragraph("Multiple Sequence Alignment Report", titleFont))
        document.add(Paragraph(" "))

        for (i in result.alignedSequences.indices) {
            document.add(Paragraph("Aligned Sequence ${i + 1}:\n${result.alignedSequences[i]}", normalFont))
            document.add(Paragraph(" "))
        }

        document.add(Paragraph("Identity: %.2f%%".format(result.identity), normalFont))
        document.add(Paragraph("Gap Count: ${result.gapCount}", normalFont))
        document.add(Paragraph("Final Score: ${result.score}", normalFont))
    ...
    fun renderMatrixToImage(
        ...
        g.font = Font("Arial", java.awt.Font.PLAIN, 18)
        val letterFont = Font("Arial", java.awt.Font.BOLD, 20)
    ...
    fun generateAlignmentSVG(
        alignment: List<String>,
        identity: Double,
        gapCount: Int,
        score: Int,
        outputDir: String = RESULTS_FOLDER,
        fileName: String = "msa_${generateTimestampedFilenameImage().replace(".png", ".svg")}"
    ) {
        ensureOutputDirectoryExists()
        ...
        val height = rows * cellSize + padding * 2 + 100 // Extra space for stats
        ...
        val document: org.w3c.dom.Document = domImpl.createDocument(null, "svg", null)
        val svgGenerator = SVGGraphics2D(document)

        svgGenerator.setSVGCanvasSize(Dimension(width, height))
        svgGenerator.font = Font("JetBrains Mono", java.awt.Font.PLAIN, fontSize)
        ...
        // Draw stats below the alignment
        val statsY = padding + rows * cellSize + 40
        svgGenerator.font = Font("SansSerif", java.awt.Font.BOLD, 16)
        svgGenerator.color = Color.BLACK
        svgGenerator.drawString("Identity: %.2f%%".format(identity), padding, statsY)
        svgGenerator.drawString("Gap Count: $gapCount", padding + 200, statsY)
        svgGenerator.drawString("Score: $score", padding + 400, statsY)
    ...
    fun showMSAVisualization(alignedSequences: List<String>, similarityPercent: Double) {
        ...
        val panel = object : JPanel() {
            ...
            override fun paintComponent(g: Graphics) {
                ...
                g2.font = Font("JetBrains Mono", java.awt.Font.PLAIN, fontSize)
        ...
        identityLabel.font = Font("SansSerif", java.awt.Font.BOLD, 16)
